...
using static ABC159.D.Cin;
using static ABC159.D.Util;
using Pair = ABC159.D.VTuple<long, long>;
...
namespace ABC159
{
    public class D
    {
        public static void Main(string[] args)
        {
            ...
            long[] acc = Accumulate(b.Select(m => nCk(m, 2)).ToArray());

            DontAutoFlush();
            for (int i = 0; i < n; i++)
            {
                long ans = (acc[a[i]] - acc[0]) + (acc[200000] - acc[a[i]+1]) + (long)nCk(b[a[i]] - 1, 2);
        ...
        /// <summary>
        /// nÇ™ëÂÇ´Ç≠ÇƒkÇ™è¨Ç≥Ç¢Ç∆Ç´Ç…égÇ¶ÇÈÅB
        /// </summary>
        /// <param name="n"></param>
        /// <param name="k"></param>
        /// <returns></returns>
