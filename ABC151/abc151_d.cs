...
using static ABC151.abc151_d.Cin;
using static ABC151.abc151_d.Util;
using Pair = ABC151.abc151_d.VTuple<long, long>;
...
namespace ABC151.abc151_d
{
    public class Program
    {
        public static void Main(string[] args)
        {
            int H = ri;
            int W = ri;
            char[,] map = ReadCharTable(H,W);

            Func<int, int, int> bfs = null;
            bfs = (h, w) =>
            {
                if (map[h, w] == '#') return 0;
                ...
                int max = 0;
                var queue = new Queue<VTuple3<int, int, int>>();
                queue.Enqueue(new VTuple3<int, int, int>(h, w, 0));

                while (queue.Any())
                {
                    var v = queue.Dequeue();
                    if (visited[v.Item1, v.Item2]) continue;
                    visited[v.Item1, v.Item2] = true;
                    ChMax(ref max, v.Item3);

                    for (int i = 0; i < 4; i++)
                    {
                        var nh = v.Item1 + DXY4[i];
                        var nw = v.Item2 + DXY4[i + 1];
                        ...
                        queue.Enqueue(new VTuple3<int, int, int>(nh, nw, v.Item3 + 1));
                    }
                }
                return max;
            };
