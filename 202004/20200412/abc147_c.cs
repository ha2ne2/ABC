...
using static _20200412.abc147_c.Cin;
using static _20200412.abc147_c.Util;
using Pair = _20200412.abc147_c.VTuple<long, long>;
        ...
        public static void Main(string[] args)
        {
            ...
            for (int mask = 0; mask < maxBit; mask++)
            {
                ...
                for (int i = 0; i < N; i++)
                {
                    if ((mask & 1 << i) == 1 << i)
                    {
                        foreach (var claim in claims[i])
                        {
                            if ((mask & 1 << (int)claim.Item1)
                                != (int)claim.Item2 << (int)claim.Item1)
                            {
                                ok = false;
                                break;
                            }
