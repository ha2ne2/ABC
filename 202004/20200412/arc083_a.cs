...
using static _20200412.arc083_a.Cin;
using static _20200412.arc083_a.Util;
using Pair = _20200412.arc083_a.VTuple<long, long>;
...
/// </summary>
namespace _20200412.arc083_a
{
    public class Program
    {
        public static void Main(string[] args)
        {
            int[] W = new int[2] { ri, ri };
            int[] S = new int[2] { ri, ri };
            int E = ri;
            int F = ri;


            // dp[i] : 文中の操作を行って作ったiグラムの砂糖水に含まれる砂糖の最大のグラム数
            int[] dp = new int[F + 1];
            bool[] ok = new bool[F + 1];
            ok[0] = true;
            for (int i = 0; i <= F; i++)
            {
                if (!ok[i])
                ...
                for (int j = 0; j < 2; j++)
                {
                    if (i + 100 * W[j] <= F)
                    {
                        dp[i + 100 * W[j]] = Max(dp[i + 100 * W[j]], dp[i]);
                        ok[i + 100 * W[j]] = true;
                    }
                }
                ...
                for (int j = 0; j < 2; j++)
                {
                    if (i + S[j] <= F &&                             // コップから溢れない
                        dp[i] + S[j] <= (i + S[j]) * E / (100d + E)) // 砂糖が残らない
                    {
                        dp[i + S[j]] = Max(dp[i + S[j]], dp[i] + S[j]);
                        ok[i + S[j]] = true;
                    }
                }
            }
            ...
            if (idx == 0)
            {
                Console.WriteLine(W.Min() * 100 + " " + 0);
            }
            else
            {
                Console.WriteLine(idx + " " + dp[idx]);
            }
        }        
    }
