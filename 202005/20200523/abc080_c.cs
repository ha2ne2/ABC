...
using static _20200523.abc080_c.Cin;
using static _20200523.abc080_c.Util;
using Pair = _20200523.abc080_c.VTuple<long, long>;
...
namespace _20200523.abc080_c
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            long[,] profits = ReadLongTable(N, 11);
            long ans = long.MinValue;

            int maxBit = 1 << 10;
            for (int mask = 1; mask < maxBit; mask++)
            {
                long now = 0;
                for (int i = 0; i < N; i++)
                {
                    int common = shopStatus[i] & mask;
                    int cnt = Count1(common);
                    now += profits[i, cnt];
                }
