...
using static _20200225.Cin;
using static _20200225.Util;
using Pair = _20200225.VTuple<long, long>;
...
namespace _20200225
{
    public class Program
    {
        ...
        {
            // Abc075_B(); 10m
            // Code_Festival_2017_Qualc_B(); 22m
            Arc054_A();
            // Arc103_A(); 58m
        }

        public static void Arc103_A()
        {
            long N = rl;
            long[] A = rla;

            HashMap<long, long> hm0 = new HashMap<long, long>();
            ...
            long hm0k = hm0.Keys.Max(k => hm0[k]);
            long hm1k = hm1.Keys.Max(k => hm1[k]);

            var hm0Vals = hm0.Values.OrderByDescending(m => m).ToList();
            long hm0a = hm0Vals[0];
            long hm0b = (hm0Vals.Count > 1) ? hm0Vals[1] : 0;
            var hm1Vals = hm1.Values.OrderByDescending(m => m).ToList();
            long hm1a = hm1Vals[0];
            long hm1b = (hm1Vals.Count > 1) ? hm1Vals[1] : 0;

            if (hm0k == hm1k)
            {
                long ans1 = N - hm0a - hm1b;
                long ans2 = N - hm0b - hm1a;
                Console.WriteLine(Min(ans1,ans2));
            }
            else
            {
                long ans = N - hm0a - hm1a;
                Console.WriteLine(ans);
            }
        }

        public static void Arc054_A()
        {
            long l = rl;
            long x = rl;
            long y = rl;
            ...
            double tokeiTime = (double)tokei / (x + y);
            double hantokeiTime = INF;
            if (x < y)
            {
                hantokeiTime = (double)hantokei / (y - x);
            }

            Console.WriteLine(Min(tokeiTime,hantokeiTime));
        }

        public static void Code_Festival_2017_Qualc_B()
        {
            long N = rl;
            long[] A = rla;
            long a = 1;
        ...
        public static int[] dxy4 = { 0, 1, 0, -1, 0 };
        public static int[] dxy8 = { 1, 1, 0, 1, -1, 0, -1, -1, 1 };
