...
using static _20200313.Joi2008yo_F.Cin;
using static _20200313.Joi2008yo_F.Util;
using Pair = _20200313.Joi2008yo_F.VTuple<long, long>;
using Graph = System.Collections.Generic.List<System.Collections.Generic.List<long>>;
using System.Text;

namespace _20200313
{
    public class Joi2008yo_F
    {
        ...
        public class Dijkstra
        {
            public int N;
            public long[] Costs;
            public HashMap<long, HashMap<long, long>> Graph;
            public Dijkstra(int n)
            {
                N = n;
                Graph = new HashMap<long, HashMap<long, long>>();
                for (int i = 0; i < N; i++)
                {
                    Graph[i] = new HashMap<long, long>(long.MaxValue);
                }
                Costs = new long[N];
            }
            ...
            public void Solve(long from)
            {
                ...
                while (pq.Any())
                {
                    ...
                    {
                        var nCost = Costs[v.Second] + edge.Value;
                        if (nCost < Costs[edge.Key])
                        {
                            Costs[edge.Key] = nCost;
                            pq.Enqueue(new Pair(nCost, edge.Key));
                        }
                    }
                }
            }
