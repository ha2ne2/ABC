...
using static _20200313.Abc051_C.Cin;
using static _20200313.Abc051_C.Util;
using Pair = _20200313.Abc051_C.VTuple<long, long>;
using Graph = System.Collections.Generic.List<System.Collections.Generic.List<long>>;

namespace _20200313
{
    public class Abc051_C
    {
        public static void Main(string[] args)
        {
            ...
            {
                // 1
                ans += new string((dx > 0) ? 'R' : 'L', Abs(dx));
                ans += new string((dy > 0) ? 'U' : 'D', Abs(dy));
                ans += new string((dx > 0) ? 'L' : 'R', Abs(dx));
                ans += new string((dy > 0) ? 'D' : 'U', Abs(dy));

                // 2
                ans += (dy > 0) ? 'D' : 'U';
                ans += new string((dx > 0) ? 'R' : 'L', Abs(dx) + 1);
                ans += new string((dy > 0) ? 'U' : 'D', Abs(dy) + 1);
                ans += (dx > 0) ? 'L' : 'R';
                
                ans += (dy > 0) ? 'U' : 'D';
                ans += new string((dx > 0) ? 'L' : 'R', Abs(dx) + 1);
                ans += new string((dy > 0) ? 'D' : 'U', Abs(dy) + 1);
                ans += (dx > 0) ? 'R' : 'L';
            }
            else if (dx == 0)
            {
                ...
                ans += new string((dy > 0) ? 'U' : 'D', Abs(dy));
                ans += "RURR";
                ans += new string((dy > 0) ? 'D' : 'U', Abs(dy)+2);
                ans += "LLU";
            }
            else if (dy == 0)
            {
                ...
                ans += new string((dx > 0) ? 'R' : 'L', Abs(dx));
                ans += "DRDD";
                ans += new string((dx > 0) ? 'L' : 'R', Abs(dx)+2);
                ans += "UUR";
            }
