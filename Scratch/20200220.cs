...
using static _20200220.Cin;
using static _20200220.Util;
using Pair = _20200220.VTuple<long, long>;
...
namespace _20200220
{
    public class Program
    {
        ...
        public static void ABC145_C()
        {
            ...
            var ps = Permutation(lst);
            var cnt = ps.Count();
            double ans = 0;
            foreach(var p in Permutation(lst))
            {
                for (int i = 0; i < N-1; i++)
                {
        ...
        public static List<List<int>> Permutation(List<int> lst)
        {
            if (lst.Count == 1)
            {
            ...
            {
                return lst.SelectMany(n =>
                    Permutation(lst.Where(m => n != m).ToList())
                        .Select(p => new List<int>() { n }.Concat(p).ToList()))
                    .ToList();
            }
        }
    }
