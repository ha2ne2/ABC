...
using static ABC157.D.Cin;
using static ABC157.D.Util;
using Pair = ABC157.D.VTuple<long, long>;
...
namespace ABC157
{
    public class D
    {
        public static void Main(string[] args)
        {
            ...
            UnionFindTree uf = new UnionFindTree((int)N);
            for (int i = 0; i < M; i++)
            {
                int a = ri - 1;
                int b = ri - 1;
            ...
            for (int i = 0; i < K; i++)
            {
                int c = ri - 1;
                int d = ri - 1;
                if (!uf.IsSameGroup(c, d)) continue;
                hs[c].Add(d);
                hs[d].Add(c);
            }
            ...
            for (int i = 0; i < N; i++)
            {
                long res = uf.Size(i) - hs[i].Count - 1;
                Console.Write(res + " ");
            }
            Flush();
        }
