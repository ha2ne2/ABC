...
using static ABC162.abc162_f.Cin;
using static ABC162.abc162_f.Util;
using Pair = ABC162.abc162_f.VTuple<long, long>;
...
namespace ABC162.abc162_f
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            HashMap<Pair, long> hm = new HashMap<Pair, long>();
            Func<int, int, long> rec = null;
            rec = (cnt,i) =>
            {
                var p = new Pair(cnt, i);
                if (hm.ContainsKey(p))
                    return hm[p];
